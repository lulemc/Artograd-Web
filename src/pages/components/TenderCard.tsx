...
import { Panel, FlexRow, FlexCell, Text, Badge, Button, Dropdown, IconContainer, LinkButton, ProgressBar  } from '@epam/uui';
...
export const TenderCard: React.FC<TenderCardProps> = ({ tender }) => {
  ...
  const TenderStatusContent: React.FC<TenderCardProps> = ({ tender }) => {
	  ...
	  switch (tender.status) {
	    case 'gathering':
	      content = <Text fontSize="14" lineHeight="18">Time to implement: {tender.expected_implementation_time}</Text>;
	      break;
	    case 'investing':
	      ...
	      content = <Text fontSize="14" lineHeight="18">{tender.budgetTotal - tender.budgetGathered} remains. </Text>;     
	      break;
	    case 'voting':
	      content = <Text fontSize="14" lineHeight="18">Voting ends on: {tender.voting_end_date}</Text>;
	      break;
	    case 'implementing':
	      content = <Text fontSize="14" lineHeight="18">Implementation progress: {tender.impl_progress}%</Text>;
	      break;
	    case 'done':
	      content = <Text fontSize="14" lineHeight="18">Total donations: {tender.donations_amount}</Text>;
	      break;
	    default:
	      content = <Text fontSize="14" lineHeight="18">Status not recognized</Text>;
	  }
  ...
  return (
    <Panel shadow background="surface-main">
      <FlexRow spacing="12" vPadding="12"  padding="12">
        ...
        <FlexCell width="auto">
          <Badge color={tender.status === 'done' ? "success" : "info" } fill="outline" caption={mapping[tender.status]} />
        </FlexCell>
      </FlexRow>
      ...
      <FlexRow spacing="12"  padding="12">
        <FlexCell grow={1}>
          <LinkButton href={"/object/"+tender.id} size="48" caption={tender.shortDescription}></LinkButton>
          <Text>
