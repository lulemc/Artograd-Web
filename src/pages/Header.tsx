...
import { BurgerButton, GlobalMenu, FlexRow, Button, Text, Panel, MainMenu, MainMenuAvatar, MainMenuButton, MainMenuIcon, FlexSpacer, FlexCell, DropdownMenuButton, DropdownMenuSplitter, DropdownMenuBody, Burger } from '@epam/uui';
    ...
    const getMenuItems = (): AdaptiveItemProps[] => {
        return [
           ...
           // { id: 'logo', priority: 99, render: (p) => <MainMenuLogo key={ p.id } href={window.location.origin} icon={ LogoSvg } /> },
            { id: 'welcome', priority: 3, render: (p) => <MainMenuButton key={ p.id } href="/" caption="Welcome" /> },
            { id: 'tendersReady', priority: 3, render: (p) => <MainMenuButton key={ p.id } href="/tenders" caption="Art objects" /> },
            { id: 'flexSpacer1', priority: 100, render: (p) => <FlexSpacer key={ p.id } /> },
            { id: 'tendersProposed', priority: 3, render: (p) => <MainMenuButton key={ p.id } href="/tenders" caption="For creative people" /> },
            { id: 'tendersVoted', priority: 3, render: (p) => <MainMenuButton key={ p.id } href="/tenders" caption="For active citezens" /> },
            { id: 'tendersFunding', priority: 3, render: (p) => <MainMenuButton key={ p.id } href="/tenders" caption="For investors" /> },
            { id: 'tendersOfficers', priority: 3, render: (p) => <MainMenuButton key={ p.id } href="/tenders" caption="For state officers" /> },
            { id: 'flexSpacer2', priority: 100, render: (p) => <FlexSpacer key={ p.id } /> },
            { id: 'signin', priority: 3, render: (p) => <MainMenuButton key={ p.id } href="/s" caption="Sing In" /> },
