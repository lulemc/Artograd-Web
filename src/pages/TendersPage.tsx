    ...
    >
      ...
      >
        <FlexRow padding="12" vPadding="12" columnGap="12">
          <Badge
            color="info"
            fill={statusFilter.gathering ? 'solid' : 'outline'}
            caption="Gathering proposals"
            onClick={() => toggleStatus('gathering')}
          />
          <Badge
            color="info"
            fill={statusFilter.voting ? 'solid' : 'outline'}
            caption="Voting for proposals"
            onClick={() => toggleStatus('voting')}
          />
          <Badge
            color="info"
            fill={statusFilter.investing ? 'solid' : 'outline'}
            caption="Looking for investments"
            onClick={() => toggleStatus('investing')}
          />
          <Badge
            color="info"
            fill={statusFilter.implementing ? 'solid' : 'outline'}
            caption="In implementation"
            onClick={() => toggleStatus('implementing')}
          />
          <Badge
            color="info"
            fill={statusFilter.done ? 'solid' : 'outline'}
            caption="Completed"
